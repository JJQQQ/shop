        ...
        <div class="page-content">
            ...
            <div class="row">
                <div class="col-xs-12">
                    ...
                    <form action="/zl_shop/admin.php/Orders/update" class="form-horizontal" method="post" role="form" enctype="multipart/form-data">
                        <input type="hidden" name="id" value="<?php echo ($_GET['id']); ?>"/>
						<div class="form-group">
                            <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> 联系人 </label>

                            <div class="col-sm-9">
                                <input name="linkman" type="text" id="form-field-1" placeholder="请输入联系人" class="col-xs-10 col-sm-3" value="<?php echo ($orders['linkman']); ?>"/>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input type="text" name="address" id="address" class="col-xs-10 col-sm-3" value="<?php echo ($addr); ?>"/>
                                <span class="help-inline col-xs-12 col-sm-7">
                                    <span id="readdress"></span>
                                </span>
                            </div>
                        </div>
						...
						<div class="form-group">
                            <label class="col-sm-3 control-label no-padding-right" for="form-input-readonly"> 邮编 </label>
                            <div action="/zl_shop/admin.php/Orders/insert" class="col-sm-9">
                                <div class="fallback">
									<input type="text" name="code" id="code" class="col-xs-10 col-sm-3" value="<?php echo ($orders['code']); ?>"/>
									<span id="recode"></span>
								</div>
                            </div>
                        </div>
						...
						<div class="form-group">
                            <label class="col-sm-3 control-label no-padding-right" for="form-input-readonly"> 电话 </label>
                            <div action="/zl_shop/admin.php/Orders/insert" class="col-sm-9">
                                <div class="fallback">
									<input type="text" name="phone" id="mobile" class="col-xs-10 col-sm-3" value="<?php echo ($orders['phone']); ?>"/>
									<span id="remobile"></span>
								</div>
                            </div>
                        </div>
						
						<div class="form-group">
                            <label class="col-sm-3 control-label no-padding-right" for="form-input-readonly"> 支付金额 </label>
                            <div action="/zl_shop/admin.php/Orders/insert" class="col-sm-9">
                                <div class="fallback">
									<input type="text" name="money" id="form-field-2" class="col-xs-10 col-sm-3" value="<?php echo ($orders['money']); ?>"/>
                                </div>
                            </div>
                        </div>
						...
						<!-- <div class="form-group">
							...
							<div class="col-sm-9">
								<input name="state" class="ace" type="radio" value="1" <?php echo ($orders['state']==1?'checked':''); ?>>
								<span class="lbl"> 未发货</span>
								<input name="state" class="ace" type="radio" value="2" <?php echo ($orders['state']==2?'checked':''); ?>>
								<span class="lbl"> 已提醒发货</span>
								<input name="state" class="ace" type="radio" value="3" <?php echo ($orders['state']==3?'checked':''); ?>>
								<span class="lbl"> 已发货</span>
								<input name="state" class="ace" type="radio" value="4" <?php echo ($orders['state']==4?'checked':''); ?>>
								<span class="lbl"> 已完成</span>
								<input name="state" class="ace" type="radio" value="5" <?php echo ($orders['state']==5?'checked':''); ?>>
								<span class="lbl"> 申请退货中</span>
								<input name="state" class="ace" type="radio" value="6" <?php echo ($orders['state']==6?'checked':''); ?>>
								<span class="lbl"> 退货完成</span>
								<input name="state" class="ace" type="radio" value="7" <?php echo ($orders['state']==7?'checked':''); ?>>
								<span class="lbl"> 申请换货</span>
								<input name="state" class="ace" type="radio" value="8" <?php echo ($orders['state']==8?'checked':''); ?>>
								<span class="lbl"> 换货完成</span>
								<input name="state" class="ace" type="radio" value="9" <?php echo ($orders['state']==9?'checked':''); ?>>
								<span class="lbl"> 申请保修</span>
								<input name="state" class="ace" type="radio" value="10" <?php echo ($orders['state']==10?'checked':''); ?>>
								<span class="lbl"> 保修完成</span>
							</div> 
						</div>
...
<![endif]-->
    ...
    <script type="text/javascript">
		...
		$('#address').blur(function(){
			...
			if(address==""){
				$("#readdress").html("地址不能为空！");
				$("#readdress").css("color","red");
				a = false;
			}else if(len >20){
				$("#readdress").html("地址不能大于20个字符！");
				$("#readdress").css("color","red");
				a = false;
			}else{
				$("#readdress").html("有效地址");
				$("#readdress").css("color","green");
				a = true;
			}
		});
		...
		$('#code').blur(function(){
			...
			if("#recode"==""){
				$("#recode").html("邮编不能为空！");
				$("#recode").css("color","red");
			}else if(len != 6){
				$("#recode").html("邮编格式错误！");
				$("#recode").css("color","red");
			}else{
				$("#recode").html("有效邮箱！");
				$("#recode").css("color","green");
			}
		});
		...
		function isPhone(str){
			...
			if(reg.test(str)){
				$("#remobile").html("有效手机号！");
				$("#remobile").css("color","green");
				e = true;
			}else{
				$("#remobile").html("111请输入11位有效手机号！");
		...
		$("#mobile").blur(function(){
			...
			if(mobile==""){
			    $("#remobile").html("手机号不能为空！");
				$("#remobile").css("color","red");
					p = false;
				
			}else if(len != 11){
				$("#remobile").html("请输入11位有效手机号！");
				$("#remobile").css("color","red");
					p = false;
			}else{
			    isPhone(mobile);
